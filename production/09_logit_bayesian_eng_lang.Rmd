---
title: "Logit models of production data with Bayesian predictive modelling - English language profile"
...
```{r}
eng_lang_syllable_f0_plot = plot(eng_lang_syllable_f0, plot = FALSE)[[1]] + labs(title = "English production across language profile", x = "F0", y = "Proportion stressed") + guides(color=guide_legend(title="Syllable"), fill = 'none') +
...
eng_dur_lang_syllable = conditional_effects(eng_model, effects = "dur_period:syllable", conditions = conditions)
```

```{r}
eng_dur_lang_syllable_plot = plot(eng_dur_lang_syllable, plot = FALSE)[[1]] + labs(title = "English production across language profile", x = "Duration (periodic)", y = "Proportion stressed") +
  guides(color=guide_legend(title="Syllable"), fill = 'none') +
  scale_color_discrete(labels=c('Penultimate', 'Ultimate')) +
  theme(plot.title=element_text(hjust = 0.5), legend.position = "bottom")

eng_dur_lang_syllable_plot$data$cond__ <- ifelse(eng_dur_lang_syllable_plot$data$cond__ == "1", "Monolingual",
ifelse(eng_dur_lang_syllable_plot$data$cond__ == "2", "L1 English",
ifelse(eng_dur_lang_syllable_plot$data$cond__ == "3", "L3 English", 99)))

eng_dur_lang_syllable_plot$data$cond__ <- factor(eng_dur_lang_syllable_plot$data$cond__, levels = c("Monolingual", "L1 English", "L3 English"))

eng_dur_lang_syllable_plot
```

```{r}
cairo_pdf("figures/eng_dur_lang_syllable_production.pdf", family = "serif", width = 8, height = 6)
eng_dur_lang_syllable_plot
dev.off()
```
